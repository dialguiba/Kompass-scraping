...
def writeProductData(textileCompaniesPage,csvWriter):
    ...
    for productBlock in productList.find_all('div', class_='prod_list'):
        ...
        name = productPage.find(
            'div', class_='blockNameCompany').h1.text.strip()
        ...
        postCode = locationData.find(
            'span', class_='spRight').contents[2].strip()
        # *PAIS
        country = addressBlock.next_sibling.next_sibling.find(
            'span', class_='spRight').text.strip()
        # *TELEFONO
    ...
    if():        
        ...
        with open(local_file_name, 'w') as csvfile:
            ...
            while(nextPageLink):
                ...
                writeProductData(textileCompaniesPage,csvWriter)
